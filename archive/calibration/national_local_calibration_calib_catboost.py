...
BEST_BASE_FILE = BASE_DIR / "outputs" / "models" / "national_local_calibration_metrics_cb_cubist.csv"
...
OUT_METRICS = OUT_DIR / "national_local_calibration_metrics_calib_catboost.csv"
OUT_PREDS = OUT_PRED_DIR / "local_calibrated_predictions_calib_catboost.csv"
...
def build_base_from_params(name, params):
    if name == "SVR_RBF":
        return Pipeline([
            ("scaler", StandardScaler()),
            ("svr", SVR(kernel="rbf", C=params.get("svr__C"), gamma=params.get("svr__gamma"), epsilon=params.get("svr__epsilon"))),
        ])
    if name == "KNN":
        return Pipeline([
            ("scaler", StandardScaler()),
            ("knn", KNeighborsRegressor(n_neighbors=params.get("knn__n_neighbors"), p=params.get("knn__p"), weights=params.get("knn__weights"))),
        ])
    ...
    if name == "Cubist" and HAVE_CUBIST:
        return cubist.Cubist(unbiased=False, auto=False, **params)
    return None
...
def run_target(target, national_csv, include_knn=False, base_hint=None):
    ...
    if base_hint:
        best_name, best_model, best_params, best_rmse = base_hint
    else:
...
def get_base_hints():
    if not BEST_BASE_FILE.exists():
        return {}
    df = pd.read_csv(BEST_BASE_FILE)
    hints = {}
    for _, row in df.iterrows():
        target = row["target"]
        name = row["best_base_model"]
        params = row["best_base_params"]
        try:
            params = ast.literal_eval(params) if isinstance(params, str) else params
        except Exception:
            params = {}
        base_model = build_base_from_params(name, params)
        hints[target] = (name, base_model, params, row["national_cv_rmse"])
    return hints


def main():
    hints = get_base_hints()

    metrics_rows = []
    preds_list = []

    m_na, p_na = run_target("na_cmolkg", NATIONAL_NA, include_knn=False, base_hint=hints.get("na_cmolkg"))
    metrics_rows.append(m_na)
    preds_list.append(p_na)

    m_esp, p_esp = run_target("esp_pct", NATIONAL_ESP, include_knn=True, base_hint=hints.get("esp_pct"))
    metrics_rows.append(m_esp)
    preds_list.append(p_esp)

    m_ph, p_ph = run_target("ph", NATIONAL_ALL, include_knn=False, base_hint=hints.get("ph"))
    metrics_rows.append(m_ph)
    preds_list.append(p_ph)

    m_cec, p_cec = run_target("cec_cmolkg", NATIONAL_ALL, include_knn=False, base_hint=hints.get("cec_cmolkg"))
