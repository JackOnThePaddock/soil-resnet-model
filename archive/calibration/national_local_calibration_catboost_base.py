...
OUT_METRICS = OUT_DIR / "national_local_calibration_metrics_catboost_base.csv"
OUT_PREDS = OUT_PRED_DIR / "local_calibrated_predictions_catboost_base.csv"
...
def run_target(target, national_csv, cat_params):
...
def main():
    # CatBoost params (tuned from earlier grid for pH; reasonable default for Na/ESP)
    cat_params = {"depth": 8, "learning_rate": 0.05, "l2_leaf_reg": 5}

    metrics_rows = []
    preds_list = []

    m_na, p_na = run_target("na_cmolkg", NATIONAL_NA, cat_params)
    metrics_rows.append(m_na)
    preds_list.append(p_na)

    m_esp, p_esp = run_target("esp_pct", NATIONAL_ESP, cat_params)
