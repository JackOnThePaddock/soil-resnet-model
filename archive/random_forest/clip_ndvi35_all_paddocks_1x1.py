...
BASE_DIR = r"C:\Users\jackc\Documents\EW WH & MG SPEIRS\SOIL Tests\exports\rf_combined_ph_cec_esp\all_paddocks_1x1"
...
def match_geom(fname, geoms):
    upper = fname.upper()
    for key in geoms.keys():
        key_safe = key.replace(" ", "_")
        if upper.startswith(key_safe + "_"):
            return geoms[key]
    return None
...
def main():
    ...
    inputs = [f for f in os.listdir(BASE_DIR) if f.endswith(".tif") and "_nonneg" not in f]
    if not inputs:
