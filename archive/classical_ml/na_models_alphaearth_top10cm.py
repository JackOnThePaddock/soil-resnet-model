...
OUT_CSV = OUT_DIR / "alphaearth_top10cm_na_model_metrics.csv"
...
def main():
    ...
    if HAVE_XGB:
        ...
        xgb_search = GridSearchCV(xgb_model, xgb_grid, cv=cv, scoring="neg_root_mean_squared_error", n_jobs=-1)
        xgb_search.fit(X, y)
        rmse, mae, r2 = run_cv(xgb_search.best_estimator_, X, y, cv)
        results.append({"model": "XGBoost", "rmse": rmse, "mae": mae, "r2": r2, "params": xgb_search.best_params_})
