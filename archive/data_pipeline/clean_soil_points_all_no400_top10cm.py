...
OUT_CSV = BASE_DIR / "external_sources" / "soil_points_all_no400_top10cm.csv"
OUT_STD_CSV = BASE_DIR / "external_sources" / "soil_points_all_no400_top10cm_standardized.csv"
OUT_KML = BASE_DIR / "external_sources" / "soil_points_all_no400_top10cm.kml"
OUT_LOC_CSV = BASE_DIR / "external_sources" / "soil_points_top10cm_location_attrs.csv"
...
def main():
    ...
    # filter: remove layers deeper than 10 cm (0.1 m) when depth is known
    ...
    depth_ok = depth_ok & (~has_depth | ((df["depth_upper_m"].fillna(0) < 0.1) & (df["depth_lower_m"].fillna(0.1) <= 0.1)))
    ...
    loc_cols = ["lat", "lon", "ph", "cec_cmolkg", "esp_pct", "na_cmolkg"]
    for c in loc_cols:
        if c not in df.columns:
            df[c] = None
    out_loc_csv = safe_write_csv(df[loc_cols], OUT_LOC_CSV)

    print(f"Wrote {out_csv} ({len(df)} rows)")
    print(f"Wrote {out_std_csv}")
    print(f"Wrote {out_kml}")
    print(f"Wrote {out_loc_csv}")
