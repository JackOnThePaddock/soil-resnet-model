...
IN_CSV = BASE_DIR / "external_sources" / "ansis_exchangeable_sodium_pct_raw.csv"
OUT_CSV = BASE_DIR / "external_sources" / "ansis_exchangeable_sodium_pct_top10cm.csv"
OUT_STD = BASE_DIR / "external_sources" / "ansis_exchangeable_sodium_pct_top10cm_standardized.csv"
...
def main():
    ...
    # numeric conversions
    for col in ["lat", "lon", "depth_upper_m", "depth_lower_m", "esp_pct"]:
    ...
    # normalize esp (z-score)
    mean = df["esp_pct"].mean(skipna=True)
    std = df["esp_pct"].std(skipna=True)
    if std and std > 0:
        df["esp_pct_z"] = (df["esp_pct"] - mean) / std
    else:
        df["esp_pct_z"] = None

    df.to_csv(OUT_CSV, index=False)
    df[["site_id", "lat", "lon", "date", "depth_upper_m", "depth_lower_m", "esp_pct_z"]].to_csv(OUT_STD, index=False)
